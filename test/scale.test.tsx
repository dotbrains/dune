...
function wideDir(count: number) {
	const dir = mkdtempSync(join(tmpdir(), 'dune-scale-tree-'));
...
describe('git output size', () => {
	test('a status listing past the default pipe buffer is not read as "no changes"', () => {
		...
		// largest output now that dune runs no diffs of its own.
		const dir = mkdtempSync(join(tmpdir(), 'dune-scale-status-'));
