...
const notesFile = () => join(mkdtempSync(join(tmpdir(), 'dune-review-store-')), 'review.json');
...
test('a reply survives the round trip', () => {
	const file = notesFile();
	const answer = note('r1', { parent: 'a', kind: 'note', body: 'fixed' });
	saveNotes('/p', [note('a'), answer], { file });
	const held = loadNotes('/p', file);
	expect(held).toHaveLength(2);
	expect(held[1]).toMatchObject({ parent: 'a' });
...
test('an agent may answer a note this session is holding', () => {
	...
	saveNotes('/p', [note('a'), note('r1', { parent: 'a' })], { file });
	// dune saves again knowing only its own note: the reply is not its to drop.
	saveNotes('/p', [note('a')], { seen: new Set(['a']), file });
