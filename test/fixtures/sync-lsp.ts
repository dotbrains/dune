...
/**
 * Enforces the protocol's open discipline the way real servers do: a
 * didOpen for a document it already holds is refused and the text it
 * carried is kept (as typescript-language-server logs "Can't open already
 * open document" and drops the new text). Completion answers one item made
 * of the word at the asked position *as this server sees it*, plus how many
 * didOpens it has received — so a client that lost sync is visible in the
 * reply: the word is stale, or the count is more than one.
 */
