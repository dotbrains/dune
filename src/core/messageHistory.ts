/**
 * Walking a list of past entries with Up/Down — the shell's history in a
 * one-line field. Position `-1` is the draft: what was typed before the walk
 * started, put back when Down reaches the end of it.
 */
...
/**
 * One press of Up (`delta` 1, older) or Down (`delta` -1, newer). Returns null
 * when the key does nothing — an empty history, or an end already reached —
 * so the caller can leave the field alone rather than rewriting it with itself.
 */
