 ...
 *
 * The remote URL is the only thing dune has to go on, so it is what decides:
 ...
 * error.
 *
 * Everything here is read-only. dune posts no comment, approves nothing and
 ...
 * other line number in dune — the forges all count from 1 and are converted
...
/** One page is what dune asks for — what it left out is reported, never dropped quietly. */
...
/**
 * The token for `kind`, from the environment. `DUNE_FORGE_TOKEN` covers the
 * host whose own variable dune cannot guess; a public repository needs none of
...
export function tokenFor(kind: ForgeKind, env: Record<string, string | undefined> = process.env) {
	for (const name of [...TOKEN_ENV[kind], 'DUNE_FORGE_TOKEN']) {
...
export function forgeFor(url: string, setting: ForgeSetting = 'auto'): ForgeResult<ForgeTarget> {
	...
	if (!kind) {
		return fail(`${remote.host} is not a forge dune knows — set "reviewForge" to name it`);
	}
	...
	if (!api) return fail(`${remote.host} is Bitbucket Server, whose API dune does not speak`);
...
async function get(
	...
	if (text.length > MAX_BYTES) return fail(`${target.host} sent more than dune will read`);
	try {
...
/** 1-based as every forge reports it, 0-based as dune counts lines. */
