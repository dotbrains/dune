...
export interface PromptModalProps {
	title: string;
	initialValue: string;
	/** Past answers Up walks back through, newest first — commit messages, so far. */
	history?: string[];
...
export function PromptModal(props: PromptModalProps) {
	...
	/** How far back Up has walked; -1 is the draft `draft` holds. */
	...
	/**
	 * A recall sets the input's value, which emits `input` right back — so only
	 * a value that is not what the walk wrote counts as typing, and typing is
	 * what ends the walk.
	 */
